...
# put all our input sentences in one tensor
digit_to_word_map = {1: "One", 2: "Two", 3: "Three", 4: "Four", 5: "Five", 6: "Six", 7: "Seven", 8: "Eight", 9: "Night"}
...
# Next, we create a function that generates batches of sentences.
# Each sentence in a batch is simply a list of integer IDs corresponding to works:
...
# Finally, we create palceholders for data
...
accurary = (tf.reduce_mean(tf.cast(correct_prediction,tf.float32)))*100
...
with tf.Session() as sess:
    ...
    for step in range(1000):
        ...
        if step % 100 == 0:
            acc = sess.run(accurary, feed_dict={_inputs:x_batch,
                                               _labels:y_batch,
    ...
    outputs_example = sess.run([outputs], feed_dict={_inputs:x_test,_labels:y_test,_seqlens:seqlen_test})
