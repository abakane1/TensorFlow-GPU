...
LOD_DIR = "logs/RNN_with_summaries"
...
def rnn_step(previous_hidden_stat,x):
    current_hidden_stat = tf.tanh(
        tf.matmul(previous_hidden_stat, Wh) +
        tf.matmul(x, Wx)+b_rnn
    )

    return current_hidden_stat
...
with tf.Session() as sess:
    train_writer = tf.summary.FileWriter(LOD_DIR +'/train',
                                         graph=tf.get_default_graph())
    test_writer = tf.summary.FileWriter(LOD_DIR + '/test',
                                        graph=tf.get_default_graph())
